  ...
  .then(series=>{
    series.forEach(s=>{
      ...
      row.innerHTML=`<div class="series-row"><span class="number">${s.number}</span><h3>${s.title}</h3><p>${s.description}</p><span class="year">${s.year}<br>＋</span></div><div class="series-content"><div class="gallery">${visibleWorks.map(w=>{
        ...
        return `<article class="work"><button data-img="${image}" data-title="${w.title}" data-meta="${meta}"><img src="${image}" alt="${w.title}" loading="lazy"></button><div class="caption"><span>${w.title}</span><span>${meta}</span></div></article>`;
      }).join('')}</div></div>`;
      list.appendChild(row);
      row.querySelectorAll('.work img').forEach(img=>{
        const setOrientation=()=>img.closest('.work').classList.add(img.naturalWidth>img.naturalHeight?'landscape':'portrait');
        img.complete?setOrientation():img.addEventListener('load',setOrientation,{once:true});
      });
