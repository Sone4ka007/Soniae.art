...
let cfg = { project: {}, artworks: [] };
let stops = [];

for (const [href, marker] of [['/wg/mobile.css', 'wgMobile'], ['/wg/launch.css', 'wgLaunch']]) {
...
const L = {
  ru: {
    tag: 'Выставка, которую перемещают зрители.', map: 'КАРТА, КОТОРУЮ РИСУЕТ ВЫСТАВКА', works: 'РАБОТЫ В ПУТИ',
    back: '← Все работы', history: 'ИСТОРИЯ ПЕРЕМЕЩЕНИЙ', move: 'ПЕРЕМЕСТИЛИ РАБОТУ?',
    intro: 'Сфотографируйте работу в новом месте и добавьте место из Яндекс Карт. Заявка придёт Соне в Telegram и появится на карте только после одобрения.',
    place: 'Название места', share: 'Скопированное из Яндекс Карт', paste: 'ВСТАВИТЬ ИЗ БУФЕРА',
    pasteHint: 'Можно вставить весь текст целиком. Ничего удалять вручную не нужно.', linkFound: 'Ссылка на Яндекс Карты найдена ✓',
    noLink: 'Ссылка не найдена. Название места всё равно можно сохранить.', clipFail: 'Браузер не дал доступ к буферу. Нажмите на поле и выберите «Вставить».',
    openY: 'Открыть Яндекс Карты ↗', send: 'ОТПРАВИТЬ НА МОДЕРАЦИЮ', sending: 'ОТПРАВЛЯЕМ…', empty: 'Работа ещё не начала путь.',
    ok: 'Спасибо. Новая экспозиция отправлена на модерацию.', walking: 'работ в пути', stops: 'экспозиций', comment: 'Комментарий',
    schematic: 'Это не обычная карта города. Она возникает только из мест, где побывали работы.', openPlace: 'Открыть место в Яндекс Картах ↗',
    imagePending: 'ИЗОБРАЖЕНИЕ РАБОТЫ', photo: 'Фото новой экспозиции', photoHint: 'Сфотографируйте работу там, где вы её оставили. Фото обязательно.',
    choosePhoto: 'СФОТОГРАФИРОВАТЬ / ВЫБРАТЬ ФОТО', photoRequired: 'Добавьте фотографию работы в новом месте.', photoReady: 'Фото готово к отправке ✓',
    badPhoto: 'Не удалось подготовить фото.', pendingNote: 'Все новые остановки проходят ручную модерацию в Telegram.', missing: 'Эта работа пока не запущена.',
    home: 'На главную галереи', setupPending: 'Приём новых остановок ещё не подключён.'
  },
  en: {
    tag: 'An exhibition moved by its viewers.', map: 'THE MAP DRAWN BY THE EXHIBITION', works: 'ARTWORKS WALKING', back: '← All artworks',
    history: 'MOVEMENT HISTORY', move: 'DID YOU MOVE IT?',
    intro: 'Photograph the artwork in its new place and add the location from Yandex Maps. The submission goes to Sonya in Telegram and appears publicly only after approval.',
    place: 'Place name', share: 'Copied from Yandex Maps', paste: 'PASTE FROM CLIPBOARD', pasteHint: 'Paste the whole copied text. You do not need to clean it up.',
    linkFound: 'Yandex Maps link found ✓', noLink: 'No link found. You can still save the place name.', clipFail: 'The browser could not read the clipboard. Tap the field and choose Paste.',
    openY: 'Open Yandex Maps ↗', send: 'SEND FOR MODERATION', sending: 'SENDING…', empty: 'This artwork has not started walking yet.', ok: 'Thank you. The new exhibition was sent for moderation.',
    walking: 'artworks walking', stops: 'exhibitions', comment: 'Comment', schematic: 'This is not a normal city map. It appears only from places visited by the artworks.',
    openPlace: 'Open in Yandex Maps ↗', imagePending: 'ARTWORK IMAGE', photo: 'Photo of the new exhibition', photoHint: 'Photograph the artwork where you left it. A photo is required.',
    choosePhoto: 'TAKE / CHOOSE PHOTO', photoRequired: 'Add a photo of the artwork in its new place.', photoReady: 'Photo ready to send ✓', badPhoto: 'Could not prepare the photo.',
    pendingNote: 'Every new stop is manually moderated in Telegram.', missing: 'This artwork has not launched yet.', home: 'Back to gallery', setupPending: 'New-stop submissions are not connected yet.'
  }
};
...
const artVisual = (artwork, card = false) => {
  ...
  if (artwork.image) return `<div class="${cls}"><img src="${esc(artwork.image)}" alt="${esc(lang === 'ru' ? artwork.titleRu : artwork.titleEn)}" loading="lazy"></div>`;
...
async function boot() {
  const [artworkConfig, routeData] = await Promise.all([
    fetch('/content/walking-gallery.json', { cache: 'no-store' }).then(r => r.json()),
    fetch('/content/walking-gallery-stops.json', { cache: 'no-store' }).then(r => r.ok ? r.json() : { stops: [] }).catch(() => ({ stops: [] }))
  ]);
  cfg = artworkConfig;
  stops = Array.isArray(routeData) ? routeData : (routeData.stops || []);
  render();
}
...
function artStops(id) {
  return stops.filter(stop => stop.artworkId === id).sort((a, b) => String(a.createdAt).localeCompare(String(b.createdAt)));
}
...
function parseYandex(raw) {
  ...
  const rest = (match ? text.replace(match[0], ' ') : text).replace(/\s*[—-]\s*(?:Яндекс\s*Карты|Yandex\s*Maps)\s*$/i, '').trim();
  const lines = rest.split(/\n+/).map(x => x.trim()).filter(Boolean).filter(x => !/^(?:Яндекс\s*Карты|Yandex\s*Maps|Поделиться|Share)$/i.test(x));
...
function mapAll() {
  ...
  cfg.artworks.forEach(artwork => {
    ...
    if (points.length) routes.push({ title: artwork.id, href: '/wg/' + artwork.slug + '/', points: points.map((x, i) => ({ ...x, markerLabel: String(i + 1) })) });
  });
...
function wire(artwork, endpoint) {
  ...
  paste.onclick = async () => {
    try { share.value = await navigator.clipboard.readText(); processYandex(); }
    catch { state.textContent = t('clipFail'); state.className = 'wg-paste-state'; }
  };
  place.addEventListener('paste', event => {
    const text = event.clipboardData?.getData('text') || '';
    if (/https?:\/\//i.test(text)) { event.preventDefault(); share.value = text; placeEdited = false; processYandex(); }
  });
  ...
  form.onsubmit = async event => {
    ...
    msg.textContent = '';
    const file = photo.files?.[0];
    if (!file) { msg.textContent = t('photoRequired'); msg.className = 'wg-message is-visible'; return; }
    ...
    try {
      const prepared = await window.WGPhotos.prepare(file);
      const body = new FormData();
      body.append('artworkId', artwork.id);
      body.append('locationLabel', locationLabel);
      body.append('mapUrl', parsed.url);
      body.append('note', document.getElementById('note').value);
      body.append('website', document.getElementById('website').value);
      body.append('formStartedAt', String(startedAt));
      body.append('photo', prepared, `${artwork.id}.jpg`);
